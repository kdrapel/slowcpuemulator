...
namespace System.IO {
	public static class FileUtils {
		...
		public static string ResolveShortcut(string file) {
			...
			((IPersistFile) link).Load(file, STGM_READ);
		...
		public static string ResolvePath(string path) {
			...
			if (File.Exists(path))
				return path;
			string temp, originalPath = path;
			for (int i = 0; i < Directories.Count; i++) {
				temp = Directories[i];
				if (path.StartsWith(Path.DirectorySeparatorChar.ToString())) {
					path = temp.EndsWith(Path.DirectorySeparatorChar.ToString()) ? temp.Substring(0, temp.Length - 1) + path : temp + path;
					if (File.Exists(path))
						return path;
				} else {
					path = temp.EndsWith(Path.DirectorySeparatorChar.ToString()) ? temp + path : temp + Path.DirectorySeparatorChar + path;
					if (File.Exists(path))
						return path;
				}
				path = originalPath;
